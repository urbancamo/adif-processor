...
public class DegreesMinutesDecimalSecondsLatLongParser implements LatLongParser {
    private final static Pattern PATTERN = Pattern.compile("(\\d+)[^\\d]+(\\d+)[^\\d]+(\\d+\\.\\d+).*([NnSs])[^\\d]+(\\d+)[^\\d]+(\\d+)[^\\d]+(\\d+\\.\\d+).*([EeWw])");
